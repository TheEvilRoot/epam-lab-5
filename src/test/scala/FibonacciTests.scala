...
class FibonacciTests extends FlatSpec {
  ...
  "Fibonacci function" should "return list [0, 1, 1, 2] using common recursion" in {
    assert(Fibonacci.findFibonacci(4, Fibonacci.fibonacci) ==
      List(0, 1, 1, 2))
  }

  "Fibonacci functions" should "return same lists" in {
    assert(Fibonacci.findFibonacci(6, Fibonacci.fibonacci) ==
      Fibonacci.findFibonacci(6, x => Fibonacci.fibonacci_tail(x)))
  }

  "Fibonacci function" should "return empty list with 0 passed" in {
  ...
  "Fibonacci function" should "return empty list with negative count passed" in {
